function Enable-ACMEProxyRedirect{
    ...
    param(
        [Parameter (Mandatory = $false,
            HelpMessage = "The hostname of the server, on which the URL rewrite rule should be enabled. If empty, the current server is used."
        )]
        $TargetServer,

        [Parameter (Mandatory = $false,
            HelpMessage = "The name of the URL Rewrite rule for the ACME proxy redirect, to be enabled"
        )]
    ...
    Write-FunctionBlock "[$($myinvocation.MyCommand)]" "Enabling ACME Proxy Redirect"
    ...
    # make the update
    $acmeStatus = Edit-ACMEProxyRedirect $TargetServer $URLRewriteRuleName $true $iisSiteName
    ...
    if(!$fro.RemoteConnectionOK){
        ...
        if(!$acmeStatus.RuleToggledSuccessfully){
            $fro.EnabledSuccessfully = $false
            $fro.FunctionSuccess = $false
            $fro.Errors += "Failed to enable URL Rewrite rule '$URLRewriteRuleName' in site '$iisSiteName'"
